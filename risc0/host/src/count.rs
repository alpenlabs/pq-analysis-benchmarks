//! Poseidon2 permutation count of succinct-receipt verification.
//!
//! A counting `HashSuite` replaces the stock `poseidon2` suite in the
//! `VerifierContext`; the verifier itself is not modified. Buckets are named
//! after the `HashFn`/`Rng` trait methods through which the permutations are
